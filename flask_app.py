...
try:
    ...
    if isinstance(model, (list, np.ndarray)) and len(model) == 1:
        model = model[0]
except:
...
def api_predict():
    ...
    age      = int(d.get('age', 5))
    edu      = int(d.get('education', 4))
    income   = int(d.get('income', 5))
    ...
    if is_model_valid(model):
        try:
            scaled = scaler.transform([features]) if hasattr(scaler, 'transform') else [features]
            prob   = float(model.predict_proba(scaled)[0][1]) if hasattr(model, 'predict_proba') \
                     else float(model.predict(scaled)[0])
